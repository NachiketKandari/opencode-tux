...
class TestSourceCodeBugs:
    """Verify known bugs exist in the actual source file on disk."""

    @classmethod
    def setup_class(cls):
        from pathlib import Path
        src_path = Path(__file__).parent.parent / "src" / "services" / "market_analytics.py"
        cls.source = src_path.read_text()
        cls.source_lines = cls.source.split("\n")

    def test_phase1_count_bug_in_source(self):
        """Line ~561 should contain 'phase1_count += 0' — the bug."""
        found = any("phase1_count += 0" in line for line in self.source_lines)
        assert found, "BUG CHECK: phase1_count += 0 not found — maybe it was fixed?"

    def test_phase1_count_never_incremented(self):
        """Verify no 'phase1_count += 1' exists anywhere in Phase 1."""
        # The only increment should be the broken += 0
        increments = [l for l in self.source_lines if "phase1_count +=" in l]
        non_zero = [l for l in increments if "+= 0" not in l]
        assert len(non_zero) == 0, (
            f"Found phase1_count increments that are NOT += 0: {non_zero}"
        )

    def test_no_try_except_in_run_method(self):
        """The run() method should have no error handling — bug confirmation."""
        # Find the run method
        in_run = False
        run_lines = []
        for line in self.source_lines:
            if "async def run(self, conn)" in line:
                in_run = True
            elif in_run and "async def " in line and "run" not in line:
                break
            elif in_run:
                run_lines.append(line)

        has_try = any("try:" in l for l in run_lines)
        has_except = any("except" in l for l in run_lines)
        assert not has_try and not has_except, (
            "BUG CHECK: run() has error handling — maybe it was added?"
        )

    def test_price_vs_sma50_never_used(self):
        """The price_vs_sma50 computed in Phase 1 is a local — never stored or used.

        It's computed on line 554 but the variable immediately goes out of scope
        when the next loop iteration starts. Unlike the original Pro*C where host
        variables persist across loop iterations, the Python local is dead code.

        (The same name appears in compute_momentum_score's parameter list, but
        that's a completely different scope — not the Phase 1 local.)
        """
        ...
        for i, line in enumerate(self.source_lines):
            if "async def run(self, conn)" in line:
                ...
                # First non-indented, non-empty line after run() is a new top-level def
                if "def " in line or "class " in line:
                    run_end = i
                    break

        if run_end is None:
            run_end = len(self.source_lines)

        # Now find "price_vs_sma50" as a standalone assignment (not a function param)
        # in the run() body, after the Phase 1 section
        phase2_start = None
        for i in range(run_start, run_end):
            if "Phase 2" in self.source_lines[i] or "phase2.start" in self.source_lines[i]:
                phase2_start = i
                break
        ...
        # Look for price_vs_sma50 used as an lvalue or reference after Phase 1
        bad_refs = []
        for i in range(phase2_start, run_end):
            line = self.source_lines[i]
            # Skip function definitions and comments
            if "price_vs_sma50" in line and "def " not in line:
                # Check if it's in a string or comment
                stripped = line.strip()
                if stripped.startswith("#") or stripped.startswith('"""') or stripped.startswith("'"):
                    continue
                bad_refs.append((i + 1, stripped))

        assert len(bad_refs) == 0, (
            f"price_vs_sma50 referenced in run() after Phase 1 (dead local): {bad_refs}"
        )
